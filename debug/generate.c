...
t_objects	*generate(t_sys *sys)
{
	...
	LOG;
	objects->camera[0] = camera_init(vec(70, 40, 0), vec(-1, 0, 0), 60, sys);
	...
	/**
		* PLANE
		ft_lstadd_back(&objects->objs, ft_lstnew(PLANE, plane_new(vec(0, -1.5,
						0),
	...
	/**
		* CYLINDER
		ft_lstadd_back(&objects->objs, ft_lstnew(CYLINDER, cylinder_new(vec(40,
						0,
						0), vec(1, 0, 0), vec(6, 10, 0), material_init(vec(0,
							200,
							0)))));
							*/
	ft_lstadd_back(&objects->objs, ft_lstnew(CYLINDER, cylinder_new(vec(30, 0,
					0), vec(1, 0, 0), vec(3, 10, 0), material_init(vec(200, 0,
						0)))));
	log_objs(objects);
	return (objects);
}
